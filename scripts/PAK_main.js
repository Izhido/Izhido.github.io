window.addEventListener("load", () => {
	...
	function buildFileList(index) {
		...
		const crcHeader = document.createElement("th")
		crcHeader.innerText = "CRC"
		headerRow.appendChild(crcHeader)

		fileListTable.appendChild(headerRow)
		...
		for (let i = 0; i < index.length; i++) {
			...
			crcCell.innerText = pak.contents[f].crc
	...
	function readFile(event) {
		...
		pakStats.innerText = pak.files.length.toString() + " files, " + pak.arrayBuffer.byteLength.toString() + " bytes, directory CRC: " + pak.directoryCrc.toString() + "."
